function excess = featureExcess(slicedvap,xrng,dim,vertexlist,area)


%needs a sliced 8-value vap and vertexlists (optional)


%% first, we split up the subvolumes and send all of them to the
%% program that calculates the generalized feature excess. remember its
...
disp('calculating excess values');
parfor_progress(numverts);
excesspervert = cell(numverts,1);
parfor i=1:numverts
    excesspervert{i} = interfacialexcess_fromdist_v2(slicedvap{i},xrng,dim,THRESH);
    parfor_progress;
end
parfor_progress(0);


%redistributing the values on per element basis

excess = zeros(100,numverts);

for el = 1:100
    
    for vert = 1:numverts
        
        excess(el,vert) = excesspervert{vert}(el)/area(vert);
    end
end
