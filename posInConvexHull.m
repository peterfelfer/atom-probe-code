function in = posInConvexHull(points,pos,tess,tol)
% inhull: tests if a set of points are inside a convex hull
% 
% usage: in = inhull(points,pos)
% usage: in = inhull(points,pos,tess)
% usage: in = inhull(points,pos,tess,tol)
%
% arguments: (input)
%  points - n*p array to test, n data points, in p dimensions
%       If you have many points to test, it is most efficient to
%       call this function once with the entire set.
%
%  pos - m*p array of vertices of the convex hull, as used by
%       convhulln.
%
...
%% converting pos to xyz
if exist('pos','var')
    if size(points,2)==2
        posForHull = [pos.x pos.y];
    else
        posForHull = [pos.x pos.y pos.z];
    end
end
...
[n,c] = size(points);
if p ~= c
  error 'points and posForHull must have the same number of columns'
end
if p < 2
...
switch p
  case 2
    ...
    nrmls = (posForHull(tess(:,1),:) - posForHull(tess(:,2),:)) * [0 1;-1 0];
  ...
  case 3
    ...
    ab = posForHull(tess(:,1),:) - posForHull(tess(:,2),:);
    ac = posForHull(tess(:,1),:) - posForHull(tess(:,3),:);
    nrmls = cross(ab,ac,2);
  ...
  otherwise
    ...
    for i = 1:nt
      ...
      %  nullsp = null(bsxfun(@minus,posForHull(tess(i,2:end),:),posForHull(tess(i,1),:)))';
      nullsp = null(posForHull(tess(i,2:end),:) - repmat(posForHull(tess(i,1),:),p-1,1))';
      if size(nullsp,1)>1
        degenflag(i) = true;
        nrmls(i,:) = NaN;
      else
...
for i = 1:blocks
   ...
   in(j) = all((nrmls*points(j,:)' - aNr) >= -tol,1)';
end
