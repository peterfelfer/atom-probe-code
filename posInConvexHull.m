function in = posInConvexHull(pos,xyz,tess,tol)
% posInConvexHull: tests if a set of points are inside a convex hull
%
% usage: posInConvexHull = inhull(pos,xyz)
...
%
%  xyz - m*p array of vertices of the convex hull, as used by
%       convhulln.
%
...
if exist ('pos','var')
    if size(xyz,2)==2
        posDimensions = [pos.x pos.y];
    else
        posDimensions = [pos.x pos.y pos.z];
    end
end
%% get array sizes
% m points, p dimensions
p = size(xyz,2);
[n,c] = size(posDimensions);
if p ~= c
  error 'posDimensions and xyz must have the same number of columns'
end
if p < 2
  error 'posDimensions must lie in at least a 2-d space.'
end
...
for i = 1:blocks
   ...
   in(j) = all((nrmls*posDimensions(j,:)' - aNr) >= -tol,1)';
end
