function Nmin = analyzeClusterSizes(clusterSizes,r_clusterSizes)
...
r_clusterSizes = sort(r_clusterSizes);
r_uniqueSizes = unique(r_clusterSizes);

for idx = 1:length(r_uniqueSizes)
    r_multiplicity(idx) = sum(r_clusterSizes == r_uniqueSizes(idx));
end
